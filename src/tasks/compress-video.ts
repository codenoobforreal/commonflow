...
export async function compressVideos(args: CompressVideosArgs) {
	...
	for (let i = 0; i < videos.length; i++) {
		...
		if (
			!metadataStr["width"] ||
			!metadataStr["height"] ||
			!metadataStr["avg_frame_rate"]
		) {
			throw new Error("get video metadata failed...");
		}

		const metadata = {
			width: Number.parseInt(metadataStr["width"]),
			height: Number.parseInt(metadataStr["height"]),
			avg_frame_rate: Number.parseInt(metadataStr["avg_frame_rate"]),
		};
